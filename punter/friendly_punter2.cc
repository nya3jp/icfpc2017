...
FriendlyPunter2::FriendlyPunter2() = default;
FriendlyPunter2::~FriendlyPunter2() = default;

framework::GameMove FriendlyPunter2::Run() {
  const int S = edges_.size();
  const int M = mines_->size();

  int remaining_moves = (num_remaining_turns() + num_punters_ - 1) / num_punters_;
  if (CanOption() && GetOptionsRemaining() > 0 && GetOptionsRemaining() >= remaining_moves) {
    return TryReplace();
  }
...
void FriendlyPunter2::SetState(std::unique_ptr<base::Value> state_in) {
  auto state = base::DictionaryValue::From(std::move(state_in));
  SimplePunter::SetState(std::move(state));
}

std::unique_ptr<base::Value> FriendlyPunter2::GetState() {
  auto state = base::DictionaryValue::From(SimplePunter::GetState());
  return state;
}

std::pair<int, int> FriendlyPunter2::FindForMine(
    int mine_index,
    const std::vector<std::vector<int>>& adj,
  ...
  for (int i=0; i<S; i++) {
    ...
    if (covered_by_any[i])
      site_values[i] += 10000 / sqrt(distance[i]);
  ...
  int best_target = -1;
  double best_score = -1;
  for (int i=0; i<S; i++) if (distance[i] != INF) {
  ...
  if (best_score == 0) {
    /*
    LOG(ERROR) << "## best_target: " << best_target;
    LOG(ERROR) << "## best_distance: " << distance[best_target];
    LOG(ERROR) << "## best_score: " << best_score;
    LOG(ERROR) << "## reachable: " << reachable[best_target];
    */
  ...
  std::pair<int, int> edge = path.back();
  if (num_remaining_turns() < rivers_->size() - num_punters_ * mines_->size() &&
      num_remaining_turns() > rivers_->size() * 0.7) {
    edge = path[engine() % path.size()];
  }
...
framework::GameMove FriendlyPunter2::TryReplace() {
  int best_score = -1;
  int best_river_index = -1;
  for (int i = 0; i < rivers_->size(); i++) {
    if (rivers_->Get(i).punter() == punter_id_ || rivers_->Get(i).option_punter() == punter_id_)
      continue;

    if (rivers_->Get(i).punter() != -1 && rivers_->Get(i).option_punter() != -1)
      continue;
